...
class LDAP implements LDAPInterface
{
    ...
    public function __construct($ldap_host, $ldap_port = 389, $ldap_version = 3, $ldap_start_tls = false, $ldap_cert_path, $ldap_key_path, $ldap_secure = false)
    {
        ...
        if ($ldap_secure === true) {
            ...
            ldap_set_option(NULL, LDAP_OPT_X_TLS_CERTFILE, $ldap_cert_path);
            ldap_set_option(NULL, LDAP_OPT_X_TLS_KEYFILE, $ldap_key_path);
        }
        ...
        if (is_int($ldap_version) && $ldap_version <= 3 && $ldap_version > 0) {
            file_put_contents("php://stderr", "LDAP Version: $ldap_version\n");
    ...
    public function getDataForMattermost($ldap_base_dn, $ldap_filter, $ldap_bind_dn, $ldap_bind_pass, $ldap_search_attribute, $user)
    {
        ...
        $uid = ldap_get_values($this->ldap_server, $data, "uid");
        if (!$uid) {
            throw new Exception('An error has occured during ldap_get_values execution (uid). Please check parameter of LDAP/getData.');
        }
        ...
        return array("mail" => $mail[0], "cn" => $cn[0], "displayName" => $displayName[0], "uid" => $uid[0]);
    }
